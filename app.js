...
var lastUpdated=document.getElementById('lastUpdated'); // ✅ 副標時間
var versionEl=document.getElementById('version');       // ✅ 副標版本
var coordList=document.getElementById('coordList');     // ✅ 座標清單
...
worker.onmessage=function(ev){
  ...
  if(data.type==='SOLVE_DONE'){
    ...
    stepList.innerHTML='';
    if (coordList) coordList.innerHTML=''; // ✅ 清空座標清單
    ...
    if (coordList && data.movesWithCoords && data.movesWithCoords.length) {
      for (var m=0; m<data.movesWithCoords.length; m++) {
        ...
        li2.textContent = (m+1)+'. moveTo('+mv.from.x+','+mv.from.y+','+mv.to.x+','+mv.to.y+')';
